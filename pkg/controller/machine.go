...
func (c *controller) addNodeToMachine(obj interface{}) {
	...
	if node == nil {
		klog.Errorf("Couldn't convert to node %+v from object", node.Name)
		return
	}
...
func isMachineStatusEqual(s1, s2 v1alpha1.MachineStatus) bool {
	...
	if (s1Copy.LastOperation.LastUpdateTime.Time.After(time.Now().Add(tolerateTimeDiff))) || (s2Copy.LastOperation.LastUpdateTime.Time.After(time.Now().Add(tolerateTimeDiff))) {
		return false
	}
	s1Copy.LastOperation.LastUpdateTime, s2Copy.LastOperation.LastUpdateTime = metav1.Time{}, metav1.Time{}

	if (s1Copy.CurrentStatus.LastUpdateTime.Time.After(time.Now().Add(tolerateTimeDiff))) || (s2Copy.CurrentStatus.LastUpdateTime.Time.After(time.Now().Add(tolerateTimeDiff))) {
		return false
	}
