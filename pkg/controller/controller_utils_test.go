...
var _ = Describe("#controllerUtils", func() {
	...
	Describe("##AddOrUpdateAnnotationOnNode", func() {
		...
		DescribeTable("##table",
			func(data *data) {
				...
				nodeObject, _ = c.targetCoreClient.CoreV1().Nodes().Get(nodeObject.Name, metav1.GetOptions{})
			...
			Entry("Error should be thrown when the node doesnt exist", &data{
				setup: setup{
					node: newNode(
						1,
						&corev1.NodeSpec{},
	...
	Describe("##RemoveAnnotationsOffNode", func() {
		...
		DescribeTable("##table",
			...
			FEntry("given annotations should be removed when there are same annotations on node", &data{
				setup: setup{
					node: newNode(
						1,
						&corev1.NodeSpec{},
			...
			FEntry("given annotations should be removed when there are no annotations on node", &data{
				setup: setup{
					node: newNode(
						1,
						&corev1.NodeSpec{},
			...
			FEntry("given annotations should be removed when there are annotations on node with different value", &data{
				setup: setup{
					node: newNode(
						1,
						&corev1.NodeSpec{},
			...
			FEntry("error should be thrown when there is no node-object", &data{
				setup: setup{
					node: newNode(
						1,
						&corev1.NodeSpec{},
